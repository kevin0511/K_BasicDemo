...
+ (NSInteger)totalDataCountForScrollView:(UIScreen *)scrollView
{
...
+ (void)addLoadMoreForScrollView:(UIScrollView *)scrollView loadMoreCallBack:(BDRefreshAndLoadMoreHandle)loadMoreCallBackBlock
{
    ...
    [footer setTitle:@"BD正在加载数据" forState:MJRefreshStateRefreshing];
...
+ (NSString *)updateTimeForTimeInterval:(NSInteger)timeInterval
{
    ...
    // 转月
    NSInteger mouths = time / 3600 /24 /30 ;
    if (mouths < 12) {
        return [NSString stringWithFormat:@"%ld月前",mouths];
    }
...
{
    if ([BDUserInfoManager sharedManager].isLogin == NO) {
        return NO;
    }
    if (userId == 0) {
        return NO;
    }
    return [BDUserInfoManager sharedManager].currentUserInfo.user_id == userId;
}
